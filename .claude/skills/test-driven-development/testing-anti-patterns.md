...
void rendersSidebar() {
    ...
    assertNotNull(page.getSidebarComponent()); // testing mock, not behavior
}
```
...
@Test
void rendersSidebar() {
    Page page = new Page(new Sidebar());  // Don't mock sidebar
...
public class SessionTestUtils {
    public static void cleanupSession(Session session, WorkspaceManager manager) {
        ...
        if (workspace != null) {
            manager.destroyWorkspace(workspace.getId());
        }
    }
}
...
void detectsDuplicateServer() {
    ...
    when(mcpServerManager.startServer(any())).thenReturn(mockHandle); // Just mock slow server startup
...
// Later: NullPointerException when code accesses response.getMetadata().getRequestId()
```
...
- Assertion checks for mock-specific test IDs or stub objects
