...
- 新增字段必须：
  ...
  3. per-turn 字段在 `app/nodes/ingest.py` 重置；入口 `app/api/turn_state.py::inputs_to_state` 不写业务对象默认值（ADR 0024 D2）；跨轮记忆只有 `history_messages` 与 `last_confirmed_params`（主图 `remember_confirmed_params` 写入，仅产品协议允许的裸确认经 `app/graph/memory.py::memory_order_ids` 读取；CWAIJY-957 互换确认下单必须引用，禁止原文单号及记忆补号；显式引用 / 单号在允许的链路优先）
