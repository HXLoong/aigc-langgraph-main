...
> 本文件取代 2026-05 的 `docs/m3-m4-roadmap.md`（M1–M4 里程碑任务图，已于 2026-09-22 退役）；不再维护任务码、owner 表与 GitHub issue 编号，完成项直接从表里划掉。

## 1. 三条主线与现状

| 主线 | 已落地（截至 2026-09-22） | 未完成 |
|---|---|---|
| **原生 LangGraph 重构** | 子图原生嵌入、`Send` 多指令并行、RetryPolicy、State 分层与 output schema（ADR 0024 / 0028）；持久化与幂等、回执、对账（0026）；字段证据契约（0027）；标的识别移交 Java（0025）；Dify 资产冻结（0024 D1） | `POST /v1/runs` 协议原生化与 Dify wire adapter 退役（需 Java 联调）；`app/tools/ticker_client.py` 去留；Store（按诉求） |
| **数据集评测与评估** | 显式验收集 `tests/fixtures/categories/`（三方言唯一加载器）；REJECTED 单独成桶、早停记失败；LLM Judge（`scripts/langfuse/langfuse_eval.py`）；节点级 fixture 与 `harness node-run`（0029） | 上线观察层基线（5xx / cascade / P95）按当前模型重测；D 桶（客户真实输入）回流与标注运营；写类 case `expected.place_params` 补齐 |
| **Harness 工程** | CI 在 push / PR 上跑 fast + full 两个 job（ruff、mypy、四项一致性 lint、全量 pytest）；PromptSpec（0023）；trace_id 贯穿与结构化日志（0004 / 0024 D5） | CI MySQL service（`tests/integration` 真实 MySQL 用例）；节点 fixture 漂移守护；VL 接线后 `llm_failure_high` 按模型拆阈值 |
...
- 待办：离线依赖包（pip wheel + docker save）；runbook 附录 A 真实 URL / 联系人回填；回切演练后 runbook 升 v1.0
