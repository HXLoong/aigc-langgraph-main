...
| **数据集评测与评估** | 显式验收集 `tests/fixtures/categories/`（三方言唯一加载器；历史 unified 仅显式选入；Excel 导出共用 categories）；REJECTED 单独成桶、早停记失败；LLM Judge（`scripts/langfuse/langfuse_eval.py`）；节点级 fixture 与 `harness node-run`（0029） | 上线观察层基线（5xx / cascade / P95）按当前模型重测（#233）；D 桶（客户真实输入）回流与标注运营（#236）；写类 case `expected.place_params` 补齐（#220） |
