...
|---|---|---|
| **原生 LangGraph 重构** | 子图原生嵌入、`Send` 多指令并行、RetryPolicy、State 分层与 output schema（ADR 0024 / 0028）；持久化与幂等、回执、对账（0026）；字段证据契约（0027）；标的识别移交 Java（0025）；Dify 资产冻结（0024 D1） | `POST /v1/runs` 协议原生化与 Dify wire adapter 退役（需 Java 联调，#222）；`app/tools/ticker_client.py` 去留（#231）；Store（按诉求） |
...
| **Harness 工程** | CI 在 push / PR 上跑 fast + full 两个 job（ruff、mypy、四项一致性 lint、全量 pytest）；PromptSpec（0023）；trace_id 贯穿与结构化日志（0004 / 0024 D5） | CI MySQL service（`tests/integration` 真实 MySQL 用例）；节点 fixture 漂移守护；VL 接线后 `llm_failure_high` 按模型拆阈值（#232） |
