...
| **数据集评测与评估** | 显式验收集 `tests/fixtures/categories/`（三方言唯一加载器；历史 unified 仅显式选入；Excel 导出共用 categories）；REJECTED 单独成桶、早停记失败；LLM Judge（`scripts/langfuse/langfuse_eval.py`）；节点级 fixture 与 `harness node-run`（0029） | ~~当前模型参考基线重测（#233）~~ 已于 2026-09-24 按本地 dry-run 回填；生产同拓扑基线与 7 天观察仍待验收；D 桶（客户真实输入）回流与标注运营（#236）；写类 case `expected.place_params` 补齐（#220） |
