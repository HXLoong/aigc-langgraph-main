...
| **资深开发 #25** | C1.3 intent v2 / C1.4 prompt 瘦身 / C1.9-12/15-18 深度业务任务 / 阶段 2 真后端联调主推手 / 阶段 3-4 主推手 / 带教 review #26 的可观测性 + 部署脚本 | #25 + #29 #30 reviewer |
...
**本周（5/12-5/16）· 阶段 1 启动：**

1. Tony 开 GitHub Issue **#PM-1**（数据集协调）+ **#PM-2**（培训 + sign-off）
2. 五条子线同时铺开：
   - 子线 1A（数据集）：#26 跑 B1.1（ticker fixture 扩到 30 条）；PM 协调启动 B1.2/B1.5；实习生 #27 写 B1.6 质量审计脚本
   - 子线 1B（代码节点）：#26 起 C1.1 swap.hand_to_share；#25 起 C1.3 swap.intent v2 prompt 调优
   - 子线 1C（可观测性）：#26 起 C1.5 业务指标埋点（#25 review）
   - 子线 1D（部署能力）：#25 + Tony 写 C1.9 客户环境调研报告
   - 子线 1E（流程文档）：Tony 起 C1.16 on-call runbook 草稿

**下周（5/19-5/23）· 阶段 1 中段同步：**
- 周一同步会：D 桶初版进度 + #26 带教任务进展
- 阻塞项识别（如有）

**三周后（约 5/30）· 阶段 1 退出门评审会**
