...
- `durability="exit"`：图内无 interrupt，单轮无需中途恢复；写路径（下单 / 平仓 / 确认）提交后端前是否额外落盘，按幂等设计裁决（[ADR 0026](./0026-request-idempotency-uncertain-receipts-reconciliation.md)）。
- 请求级幂等：路由层以 `(conversation_id, message_id)` 去重，启用 `message_log.uk_message_id`，冲突即回放上一次 `reply_text`，不重跑图；`node_trace` 加幂等键；schema 演进引入 alembic。**落地形态与不确定写入 / 对账裁决见 [ADR 0026](./0026-request-idempotency-uncertain-receipts-reconciliation.md)**。
...
### D8 · 门槛（2026-09-22 修订：分阶段表退役，统一为 [ADR 0030](./0030-goal-restatement-native-langgraph-dataset-eval-harness.md) D3）

原表把重构拆为"通电与止血 / State 契约 / 子图原生化 / 协议原生化 / 持续"五段并各附周数与门槛。里程碑口径退役后，工作包保留（见文末落地记录），门槛只有一套：每批改动 pytest GREEN、数据集 PASS 率不低于前值、trace 每步可归因、CI 在 PR 上跑；无 LLM 密钥的环境只能跑 pytest 与确定性 harness 断言，不得代替 eval 门。未完成的工作包：`POST /v1/runs` 协议原生化与 Dify wire adapter 退役（需 Java 联调）、D 桶回流与覆盖矩阵、Store（按诉求）。
...
- **先做子图原生化再修评估台**：无守护的重构，评估报告第五节已证明现役集守不住 `place_params` 与多轮。否决，评估台通电必须先行。

## 后果

- 正面：图、State、持久化、可观测、评估五层各有一份显式契约；金融三条 P0（单连接 saver、无幂等、敏感字段出境）在首批关闭；Dify 从"上游真源"变为历史参照，后续同步事故（SW-INC-01 类）不再可能发生。
