...
- 作者：图灵科技 + Tony
- **冻结（2026-09-22）**：本 ADR 不再追加落地记录，后续决策一律开新编号——已拆出：[ADR 0025](./0025-instrument-resolution-delegated-to-backend.md)（标的移交 Java，撤销 D3 ticker 真子图）、[ADR 0026](./0026-request-idempotency-uncertain-receipts-reconciliation.md)（D4 留白的幂等 / 回执 / 对账裁决）、[ADR 0027](./0027-field-evidence-contract.md)（字段证据契约）、[ADR 0028](./0028-session-entry-and-multi-instruction-send-orchestration.md)（入口分流与多指令 `Send` 编排）、[ADR 0029](./0029-node-level-debug-api-and-regression-workbench.md)（D6 之下的节点层）。D8 阶段 0 的"CI 在 PR 上跑"门槛于 2026-09-22 补齐：`ci.yml` 恢复 push / pull_request 触发，fast job（ruff + 同步 / 阈值 / fixture / ADR 四项 lint + 单测）→ full job（全量 + mypy）；CI MySQL service 仍是待办。
