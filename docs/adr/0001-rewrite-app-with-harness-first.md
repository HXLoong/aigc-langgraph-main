...
3. **harness 只 import `app.graph.main.build_main_graph`** —— 主链路成立；例外见"实现偏离"
...
- **人读** = LangFuse UI
- **suspected_node / suspected_prompt** = `harness/reporter.py` 启发式层（"diff 字段的最后写入节点" + state 字段→节点→prompt 映射表）
