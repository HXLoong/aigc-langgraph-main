...
- 新增意图/子图必须同步更新 `app/graph/state.py` 的 TypedDict（M1 兼容 shim 已于 2026-09-17 随 ADR 0024 删除，入口唯一路径为 `app/api/turn_state.py`）与 `docs/archive/fixtures/old_typing/golden.jsonl`——后者已由 CI 的 `scripts/check_fixture_consistency.py` 强制。
