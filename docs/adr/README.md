...
本目录记录场外衍生品 AI 指令助手（otc-agent）的全部架构决策：**ADR 0000-0030（共 31 篇）**，其中现行 23 篇、历史存根 8 篇。
...
| 交易确认 | 文本二阶段确认：明确动作 + 引用当前订单才执行写操作 | 写操作都有客户显式二次表达，不误触发 | [0021](./0021-text-confirm-replaces-interrupt.md) · [0006](./0006-hitl-interrupt-boundary.md) |
...
| [0006](./0006-hitl-interrupt-boundary.md) | HITL 边界：写 + 资金双轴 | interrupt 部分被 0021 取代；风险规则有效 | — |
...
| [0010](./0010-llm-model-selection-rules.md) | Qwen 三型号分工 | 已被 0020 取代 | 历史存根 |
...
| [0013](./0013-load-dynamic-inference-prompt-fragment.md) | 加载后端动态 prompt 片段 | 已撤销 | 历史存根 |
...
| [0016](./0016-m3-scope-engineering-loop-not-shadow.md) | 迁移验收改为工程联调闭环 | 已被 0030 取代 | 历史存根 |
| [0017](./0017-m4-canary-quantitative-exit-gate.md) | 金丝雀切流退出门量化指标 | 已被 0030 取代 | 历史存根；指标并入 0030 D3 |
| [0018](./0018-dev-qwen-prod-deepseek-llm-split.md) | 开发 Qwen / 现场 DeepSeek 双模型 | 已被 0020 取代 | 历史存根 |
| [0019](./0019-incident-severity-thresholds.md) | 故障升级阈值 P0 / P1 / P2 | 已采纳 | 与 0030 D3 互补 |
| [0020](./0020-unify-all-llm-on-deepseek-v4-pro.md) | 全量统一 DeepSeek-V4-pro | 已采纳 | 取代 0018、0010 |
| [0021](./0021-text-confirm-replaces-interrupt.md) | 文本二阶段确认 + checkpointer 接线 | 已采纳 | 取代 0006 interrupt 部分 |
| [0022](./0022-prompt-governance-after-code-migration.md) | 迁移后的提示词治理模型 | 已废弃 | 历史存根；由 0023 承接 |
...
| [0025](./0025-instrument-resolution-delegated-to-backend.md) | 标的识别移交 Java 后端 | 已采纳（追认） | 取代 0008 |
...
| [0030](./0030-goal-restatement-native-langgraph-dataset-eval-harness.md) | 目标重述与统一评测门 | 已采纳 | 取代 0016 / 0017 |
...
| 已被 NNNN 取代 / 已撤销 / 已废弃 | 改写为历史存根：原决策 + 为何失效 + 仍有效的事实；**不删文件、不重编号**（交叉引用经不起断） |

被修订而非取代的 ADR 状态不变，在"关系"行注明修订来源；本表状态须与各篇头部一致。
