...
| 遗留（待裁决） | `app/tools/ticker_client.py` 在 `app/` 内已无调用方（仅 `app/tools/__init__.py` 导出），去留待定；`app/config.py` 的 `ticker_mysql_*` / `securities_instrument_*` 死配置（ADR 0012 遗留项）仍未清理 |
