# ADR 0028 · 会话保护入口分流与多指令 Send 编排

- 状态：已采纳（多指令编排 2026-09-18 落地 commit `8fab943`；会话保护与三分支入口 2026-09-21 `f8dda1e`；本篇为 2026-09-22 追认记录）
...
DSL v2 迁移后主图入口是"快速询价 / 存量兼容 / 普通智能指令"三分支，但会话过期检查、入口异常与业务分流混在同一段条件路由里，且客户一条消息里带多条指令（"买入 A，然后撤销 B"）时只能整条当一个意图处理。ADR 0024 D3 把 `Send` 用在 ticker 子图上，该子图已随 [ADR 0025](./0025-instrument-resolution-delegated-to-backend.md) 删除；`Send` 在本仓的现役用途转为多指令并行准备。

## 决策

### D1 · 主图拓扑：会话保护独立于业务入口（`app/graph/main.py`）

```
START → ingest（一轮边界 + 会话活动检查）
  → [route_after_ingest] error 或 session_status=expired → render
  → entry_route（纯函数 select_entry_branch，不调模型 / 后端，写 TraceEntry）
      quick_inquiry | existing_command_query | pre_route（普通智能指令）
  → plan_instructions → [route_after_plan] instructions | pre_route | fallback
...
- 会话空闲超过 `conversation_idle_timeout_seconds`（默认 1800s）→ `ingest` 写 `session_status=expired`，直接进 `render`，不进入任何业务分支。
- 入口选择由 `app/nodes/entry_route.py::select_entry_branch` 一个纯函数决定，图边与 trace 用同一函数，避免两处漂移。

### D2 · 多指令计划：确定性预筛 → LLM 拆分 → Code 定位与拒绝规则（`app/graph/instructions.py`）
...
### D3 · `instructions` 子图：依赖分波、`Send` 并行准备、批量一次提交
