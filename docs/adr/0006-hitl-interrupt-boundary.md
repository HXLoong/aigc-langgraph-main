...
- 状态：**interrupt 机制部分已被 [ADR 0021](./0021-text-confirm-replaces-interrupt.md) 取代**（2026-08-27 裁决：文本二阶段确认为正式机制）；本文的"写 + 资金双轴"风险象限规则仍沿用
- 日期：2026-05-10
...
2026-08-27 核查确认本 ADR 的执行载体**一项都不存在**：
...
**裁决结果**（2026-08-27）：选 (b) + checkpointer 拆开接线——详见 [ADR 0021](./0021-text-confirm-replaces-interrupt.md)。新增节点仍须按上表 review 风险象限（规则本体有效），确认机制统一走文本二阶段（`confirm_*` 意图），不写 `interrupt_before`。
