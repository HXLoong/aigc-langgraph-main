...
| **P1** | · Cascade fail 率 ≥ 5% 持续 10 分钟<br>· LLM 失败率 ≥ 10% 持续 5 分钟<br>· P95 延迟 ≥ M2 baseline × 3 持续 10 分钟 | **15 分钟内介入** | 诊断根因 + 评估是否需回滚；通知工程负责人 |
...
本地标的消歧卡片（HITL）已随 ADR 0025 标的识别移交 Java 而退役，本场景不再发生；标的识别问题按 §5.4 后端链路排查。
