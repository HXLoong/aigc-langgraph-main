...
| case-031/3（原第2轮） | 引用合约、1000000、最大跟量意图 |
| case-034/3（原第2轮） | 引用合约、2000000、TWAP14:30–14:50、限价10 |
| case-034/6（原第5轮） | cancel_close确认及引用完整CO订单范围 |
...
## 2026-09-24 指定平仓合约使用运行时持仓

case-031、case-034、case-034-lifecycle 增加首轮“查可平持仓”，下一轮通过 `{{previous_holding_contract_id:1}}` 选择上一轮真实卡片的第一笔合约，再执行原有补参、确认和撤单步骤。HTTP 与 Langfuse 评测入口共用解析器；未给序号时要求唯一合约，空结果、越界、非法序号和示例内容均停止执行。意图集同步派生新增的查询轮，不删除确认/撤单或最终状态断言。用例仍为391条，业务计划轮次由422增至425。

原固定 OPT-AAAA1 以及 case-030 首轮固定合约清单并非任何环境都存在，改为验证真实持仓卡片结构；合约身份用当前引用 `$ref` 精确断言，不能把不同合约统一模糊化。原第一批12轮断言中，case-031/2 映射为第3轮，case-034/2、/5 映射为第3、6轮。全平本金、POV、TWAP、确认范围和撤单终态等业务断言保留。

## 平仓订单与合约查询记录的身份边界

新增真实持仓查询轮后，复现了 `query-close-orders` 同时返回 CO 订单记录和同合约未绑定订单的持仓记录时的混淆。现在：原文或当前引用已明确 CO 订单时，只按该订单号匹配并核验合约一致；原文只给合约时，当前引用若唯一绑定一个 CO 则沿用，引用存在多笔同合约 CO 则拒绝；没有引用绑定时优先采用该合约未绑定订单的持仓记录。金额计算使用同一条匹配记录，不再取第一个“订单号或合约号相同”的行。

三项 RED 分别证明误报多记录、取错同合约余额、漏掉订单/合约冲突；修复后平仓子图296项通过，仍保留多笔同合约引用须明确订单/序号的拒绝。Java 后端、查询 DTO 和图拓扑未修改。
