...
F4 灰度上线（F4.2-F4.5）期间，**Tony / 业务方 / on-call** 三方共用的可视化观测台。
...
如果只能看一个面板：**第 3 行的 `is_canary=false · 1h ❌`** —— 任何非零值都意味着企微管理员误把非白名单群切到了 LangGraph，对应 `alerts.py: non_canary_traffic` P0 即时告警。
...
| 灰度切流判定（合规 vs 误切） | `python scripts/canary_status.py --strict`（带退出码可用于 CI / on-call 脚本判定） |
...
- D2.6 #72：/ready endpoint + health_probes
- G5.1 / F4.2-F4.5：金丝雀切流（CANARY_ROOM_IDS）
