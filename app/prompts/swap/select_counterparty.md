...
第一步·对手选择信号门禁(防误判, 极重要)：raw(去@/机器人名后)须确出现下列之一才算“有对手信号”：
  ...
  ③ raw 里逐字符出现 shortname_list 中某项的完整 shortName，或明确交易对手位置的非空连续名称片段 A 只被唯一一个 shortName 包含。
  ④ “交易对手:/账号/交易账号” 前缀后的名称。
  ⑤ 换手动词：“换为/更换/改成/对手用 … + 交易对手/对手/账号 + 名称”。
若以上全不满足 → {"hasSignal":false,"picks":[]}，绝不从列表里自挑任何项。
【名称简写机械规则】完整 shortName 匹配优先。没有完整匹配时，只允许把用户仅回复的名称片段、订单开头或结尾的名称片段、或明确对手前缀后的名称片段作为非空 A；A 不设固定长度，只做逐字符连续子串匹配，只认 shortName、不用 longName，且不得从标的、代码、方向、数量、金额、价格、比例、时间、算法、订单序号或@提及内部截取。候选集合 M={shortName|A 是其连续子串}；|M|=1 才是唯一简写，|M|=0 或 |M|>1 均不得选择任何候选，更不得按列表顺序取第一项。
【铁律】用户只给“裸数字”而没有任何 对手字母/对手前缀序号/简称/换手动词 → hasSignal=false(那是在切标的或补价格，与对手无关)。

有信号 → hasSignal=true，逐个解析：选项字母→letter(转大写)；带前缀序号→ordinal；直报完整简称、唯一简写 A 或换手动词后的名称→directName，其中 directName 保留 raw 中实际名称片段，不自行改写为完整 shortName，由下游 code 复核唯一性。letter/ordinal/directName 三选一填一个、其余 null。
