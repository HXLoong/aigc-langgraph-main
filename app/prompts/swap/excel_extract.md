...
3. **history_query_str** (历史对话):
   - 历史对话字符串,包含完整对话历史,包括:
     * 用户的历史消息
     * 机器人的历史回复(包括订单详情、确认请求等)
     * 上下文中的订单信息(标的、数量、价格、算法等)
     * **【重要】历史LLM识别结果**:包含之前识别的完整JSON结果,用于参数补充场景的订单整合
...
4. **格式检查清单**:
   - [必须] JSON对象直接开始,第一个字符是`{`
   - [必须] JSON对象直接结束,最后一个字符是`}`
   - [必须] 没有任何前导文字或标记
   - [必须] 没有任何后缀文字或注释
   - [必须] 没有Markdown代码块符号(```)
   - [必须] 整个输出内容可以直接被JSON.parse()解析
