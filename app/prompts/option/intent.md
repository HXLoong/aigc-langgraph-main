...
1. raw_content: 用户的原始消息内容，来自企微用户

2. quote_content: 用户引用的群消息内容（可能为空）

3. history_query_str: 历史对话字符串，包含完整对话历史，包括:
   - 用户的历史消息
   - 机器人的历史回复（包括期权询价详情、确认请求等）
   - 上下文中的订单信息（单号、标的、期限、行权价格、期权费率等）

4. bot_name_list: 当前可用的机器人名称列表，用于过滤识别时排除机器人@符号的干扰
...
1. 预处理步骤: 从 raw_content、quote_content 中移除所有出现在 bot_name_list 中的机器人名称及其 @ 符号
2. 过滤范围:
   - 精确匹配: `@机器人名称` 格式
...
关键强调: 机器人名称过滤是所有意图识别的第一步，过滤后的内容才是有效输入；绝对不允许将机器人名称识别为任何业务参数
...
- 【引用消息豁免 — 优先级高于上面两条】若 quote_content 非空，且其中含订单号（“Q-”开头）或期权报价要素（行权价%／期限如1M-12M／“期权”／“看涨”／“看跌”），则当前 raw_content 是用户对该已有报价/订单的补参或操作（补金额／价格类型／算法／交易对手／期限／行权价，或回复交易对手选项字母A/B/C），即使 raw_content 单看简短或不含产品关键词，也绝不可判 unknown_intent；必须跳过本有效性门禁，继续按规则0–规则5判定。注意：本豁免不改变各意图的数据源约束（见【数据源使用优先级与规则】）
...
- confirm_cancel_order(确认撤单):
  ...
  关键区别: "撤单" -> request_cancel_order(请求阶段)；"确认撤单" -> confirm_cancel_order(确认执行阶段)。绝不能混淆
...
1. 若 type=confirm_order / confirm_cancel_order，但 raw_content 未原样包含"确认下单" / "确认撤单"四字 → 否决并改判（同规则0硬门槛）
2. 若 type=request_cancel_order，但 raw_content 不含"撤"字且不满足“raw_content 包含取消订单 + 上下文已确认下单/等待交易员审核/交易中/已有正式送出订单” → 否决；若 raw_content 为"取消订单"/"取消"且上下文仍处于待确认下单或未确认下单阶段，则改判为 cancel_order_request；若是对已有订单（含"Q-"单号）改参数（改期限/行权价/数量/执行价）则改判为 place_order_from_quote，否则改判为 unknown_intent
