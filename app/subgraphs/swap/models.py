...
class SwapOrderItem(WireModel):
    ...
    place_order_algorithm_type: Annotated[SwapAlgorithmType | None, CandidateDescription('只提取明确算法名称：POV/跟量、TWAP/全天均价、VWAP等；名称与数字比例分开提取。最大跟量、尽快、ASAP属于快速执行意图，不是算法名。均价或均價后直接跟价格数字时表示价格信息，不作为算法。算法与执行风格同时出现时不遗漏明确算法名；value 不附带风格、比例或价格。没有明确算法则为 null；不补默认算法或比例，不转枚举')] = Field(default=None, alias="placeOrderAlgorithmType", description="算法：POV / TWAP / VWAP / ICEBERG / SNIPER")
