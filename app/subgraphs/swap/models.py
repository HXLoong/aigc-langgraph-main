...
class SwapOrderItem(WireModel):
    ...
    place_order_wind_code: Annotated[str | None, CandidateDescription('证券表达原文，名称加代码可取完整表达、单独名称或单独代码，均保留各自全部字符；不包含相邻数量、交易对手或执行修饰语，证券识别由后端完成')] = Field(default=None, alias="placeOrderWindCode", description="标的原文（代码或名称片段，逐字保留；最终证券识别由后端负责）")
    place_order_transaction_type: Annotated[SwapTransactionType | None, CandidateDescription('本轮明确的交易品种原文，如港股、美股、深港通；港股不等于深港通或沪港通，不推断通道，不根据证券代码或名称推断市场；保留原词，不转枚举')] = Field(default=None, alias="placeOrderTransactionType", description="交易品种：A_SHARE / HK_STOCK / US_STOCK / SZ_HK_CONNECT / SH_HK_CONNECT / CHN_FUTURE / CROSS_FUTURE")
    ...
    place_order_order_direction: Annotated[SwapOrderDirection | None, CandidateDescription('仅本轮明确买卖或开平仓动作原文，含本笔独立的方向缩写；evidence 用连续原文证明本笔动作，不从 quote/history 复制。持仓范围、数量正负不作为方向；未明确动作时为 null，不转枚举')] = Field(default=None, alias="placeOrderOrderDirection", description="方向：BUY 买入 / SELL 卖出 / SHORT_OPEN 卖空 / SHORT_CLOSE 平空")
    place_order_price_type: Annotated[SwapPriceType | None, CandidateDescription('仅限价或市价类型原文，含对应英文缩写；均价、跟量属于算法策略，不填本字段。未明确价格类型为 null，不从算法推导，不转枚举')] = Field(default=None, alias="placeOrderPriceType", description="价格类型：LimitOrder 限价 / MarketOrder 市价")
    place_order_algorithm_type: Annotated[SwapAlgorithmType | None, CandidateDescription('算法名或均价、跟量策略原文（POV、TWAP、VWAP、ICEBERG、SNIPER 及中文策略表达）；原文同时出现算法名和执行风格时取算法名，不遗漏或以风格替代算法；不附带比例或执行时段，比例填跟量比例字段；保留原词，不转枚举')] = Field(default=None, alias="placeOrderAlgorithmType", description="算法：POV / TWAP / VWAP / ICEBERG / SNIPER")
    ...
    place_order_entrust_ratio: Annotated[float | int | None, CandidateDescription('本轮本笔委托或平仓的范围、比例原文，保留全部或部分持仓表达；evidence 必须在同一连续原文内证明范围或比例用于本笔动作，不能借用其他订单或 quote/history 的动作；单纯描述持仓状态为 null，不计算比例，不填股数')] = Field(default=None, alias="placeOrderEntrustRatio", description="委托比例（可与 placeOrderQuantity 同时非 null）")
