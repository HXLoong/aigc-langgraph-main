...
CO- 单号与合约编号的正则单一来源在 app/domain/order_ids.py；本模块的 ORDER_ID_RE
（大小写不敏感，输出统一大写）与 CONTRACT_CODE_RE 为其别名。
...
- confirm / confirm_cancel（最终确认）：走 app/domain/confirmation.py，不用本模块
...
from app.domain.numerals import ordinal_int
from app.domain.order_ids import CLOSE_ORDER_ID_RE, CONTRACT_CODE_RE

#: 用户输入中的平仓订单号（大小写不敏感，输出统一大写）
ORDER_ID_RE = CLOSE_ORDER_ID_RE
...
def _specified_signals(raw: str) -> tuple[list[str], list[int], list[str], list[int]]:
    ...
    ordinals = list(dict.fromkeys(
        ...
        if (value := ordinal_int(match.group(1))) is not None
    ))
...
def _label_mapping(quote: str, quote_ids: list[str]) -> dict[int, str | None]:
    ...
    引用无序号标签时按出现顺序编号（与 app/domain/confirmation.py 一致）；
    ...
    for index, marker in enumerate(markers):
        value = ordinal_int(marker.group(1))
