...
- option.extract_place · 请求下单参数提取（place_order_from_quote，确定性，去 LLM）
- option.extract_confirm_place · 确认下单订单号+补参提取（confirm_order，确定性，去 LLM）
- option.extract_cancel_place · 取消下单订单号提取（cancel_order_request，确定性，去 LLM）
- option.extract_cancel · 请求撤单订单号提取（request_cancel_order，确定性，去 LLM）
- option.extract_confirm_cancel · 确认撤单订单号提取（confirm_cancel_order，确定性，去 LLM）
- option.extract_query · 查询参数提取（query_order_status，确定性，去 LLM）
"""
