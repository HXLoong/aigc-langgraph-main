...
metadata:
  source: .claude/skills/iterate-option/SKILL.md
  argument_hint: '[--limit N] [--filter 期权询价] [--dry-run]'
---

> 自动生成自 `.claude/skills/iterate-option/SKILL.md`（python scripts/sync_agents_md.py），禁止手改。
...
调用 `.claude/skills/test-driven-development/SKILL.md` 流程，按其 workflow 执行。**禁止跳过。**
