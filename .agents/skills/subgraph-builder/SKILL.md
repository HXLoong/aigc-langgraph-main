...
- ADR 0007（新场景归属：独立子图 vs 现有子图新意图）、ADR 0024（原生重构目标架构）
...
START → _route_swap_entry（文本 / 图片 / Excel）
      → <p>_intent（Code 规则优先，未命中才调模型）
      → _route_after_<p>_intent（common.intent_router）：先 has_error → <p>_unknown，再查 _INTENT_TO_NODE
      → 按意图的参数节点（Code 纯计算，或 LLM 抽原文候选 → Code 归一化）
...
- 意图 / 参数的 LLM 节点：`PromptSpec` + `with_structured_output(<Output>)`，参数用 `candidate_model(<Canonical>)`
  抽原文候选，最终值由 Code 归一化节点决定；只读 IO 节点 `@io_node` 并用 `add_io_node` 注册
...
`_INTENT_TO_NODE`（条件边由 `add_intent_dispatch` 生成）→ `app/node_execution/catalog.py` 与 `app/observability/node_labels.py` 登记 → 测试。
...
3. 一级路由：`app/nodes/route_rules.py`（规则层）+ `app/nodes/intent_route.py`（ADR 0015）
