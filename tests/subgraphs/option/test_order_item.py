...
class TestOptionOrderItem:
    ...
    def test_all_fields(self) -> None:
        ...
        assert item.stockCode == "600519.SH"
        assert item.twapStartTime == "09:30"
        assert item.twapEndTime == "15:00"
        assert item.shortName == "11125测试短名（张天琪专用）"
    ...
    def test_option_type_restricted_to_three_values(self) -> None:
        ...
        for valid in ("欧式看涨", "参与型看涨", "雪球"):
            assert OptionOrderItem(optionType=valid).optionType == valid  # type: ignore[arg-type]
...
class TestOptionOrderItemWithFastExec:
    ...
    def test_has_fast_execution_intent_field(self) -> None:
        ...
        assert item.hasFastExecutionIntent is True

    def test_default_none(self) -> None:
        item = OptionOrderItemWithFastExec()
        assert item.hasFastExecutionIntent is None
