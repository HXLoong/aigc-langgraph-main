"""主图三个条件路由的纯函数单测（此前只有入口 `_route_entry` 与 cascade e2e 覆盖）。
...
from app.graph.main import _route_after_ingest, _route_after_intent, _route_after_plan
...
@pytest.mark.parametrize(
    ...
    [
        ({}, "pre_route"),
        ({"sub_instructions": []}, "pre_route"),
        ({"sub_instructions": [{"text": "a"}]}, "pre_route"),
        ({"sub_instructions": [{"text": "a"}, {"text": "b"}]}, "instructions"),
        ({"sub_instructions": [{"text": "a"}, {"text": "b"}], "error": _ERR}, "fallback"),
    ],
    ids=["missing", "empty", "single", "multi", "error_wins"],
)
def test_route_after_plan(state: dict, expected: str) -> None:
    assert _route_after_plan(state) == expected
