"""Langfuse Code Evaluator：回复至少包含一个候选文本。"""
...
def evaluate(ctx: Any) -> EvaluationResult:
    ...
    for index, expected_turn in enumerate(expected_turns):
        candidates = _lines(expected_turn.get("response_contains_any"))
        if not candidates:
            continue
        assertion_count += 1
        ...
        if not any(_normalize(candidate) in normalized_reply for candidate in candidates):
            failures.append(
                f"第 {index + 1} 轮未命中任一候选文本：{'、'.join(candidates)}"
            )
    ...
    return EvaluationResult(
        scores=[
            Score(
                name="det_required_any_text_pass",
                value=passed,
                data_type="BOOLEAN",
                comment="每轮均命中至少一个候选文本" if passed else "；".join(failures),
                metadata={
